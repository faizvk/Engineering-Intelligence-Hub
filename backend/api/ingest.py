...
async def ingest(req: IngestRequest, db=Depends(get_db), p: Principal = Depends(current_principal)) -> dict:
    row = await db.execute(
        text(
            """INSERT INTO ingest_jobs (source_uri, doc_type, content, acl)
               VALUES (:uri, :dt, :content, :acl) RETURNING id"""
        ),
        {"uri": req.source_uri, "dt": req.doc_type, "content": req.content, "acl": req.acl or ["all"]},
    )
    job_id = row.scalar()
    await db.commit()
...
async def job_status(job_id: int, db=Depends(get_db), p: Principal = Depends(current_principal)) -> dict:
    row = await db.execute(
        text("SELECT status, error FROM ingest_jobs WHERE id = :i"), {"i": job_id}
    )
    r = row.first()
    if r is None:
